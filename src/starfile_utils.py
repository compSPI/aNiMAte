...
def create_starfile(dataloader, config, root_path, relative_mrcs_path_prefix, star_file):
    ...
    with tqdm(total=config.simul_num_projs) as pbar:
        for step, (model_input, _) in enumerate(dataloader):
            ...
            defocusU = model_input['defocus_u'].float().numpy()  # B, 1, 1
            defocusV = model_input['defocus_v'].float().numpy()  # B, 1, 1
            angleAstigmatism = model_input['angleAstigmatism'].float().numpy()  # B, 1, 1
            shiftX = model_input['shift_x'].reshape(B, 1, 1).float().numpy()  # B, 1, 1
            shiftY = model_input['shift_y'].reshape(B, 1, 1).float().numpy()  # B, 1, 1
            ...
            for i in range(B):
                ...
                rlnRandomSubset.append(randint(1, 2))
    ...
    df = {'optics': pd.DataFrame(optics), 'particles': pd.DataFrame(particles)}
